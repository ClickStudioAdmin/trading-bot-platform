...
- Admin Wallet Ledger sample rows (`20260915100000_admin_wallet_ledger_demo.sql`) on `click.studio.admin@gmail.com` (else the first active admin): deposits, plan payments, Main withdraw request + reject, a paid withdraw, a correction, and an Affiliate → Main transfer that covers a monthly shortfall. Idempotent on `click-admin-ledger:` external ids. Do not merge to `main`.

Cleanup matches the `@tbp-john-demo.invalid` email domain, `John demo` campaigns, `JDEM*` URLs, and `john-demo:` payout / invoice ids. Admin ledger samples use `click-admin-ledger:` — delete those wallet rows and invoices separately if you clear the demo.
