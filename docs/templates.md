...
Phase 11 is complete. See [phase-11.md](phase-11.md). This is a **login-scoped recipe library** that stamps idle automations onto a desk of the matching type. Migration: `supabase/migrations/20260828100000_automation_templates.sql` (applies on push to `develop`).
...
| --- | --- | --- |
| User template / set | Owning member; admins may rename, describe, or delete from `/admin/templates` | Owner applies to **their** desks. Other members cannot see it. Admins see it on the admin page only. |
| Platform template / set | Admin only | Every member can read and apply. Members cannot edit or delete. |

No member-to-member sharing. Platform is the share channel. Admin support is not sharing: it does not put a user’s recipe on another member’s `/account/templates`.
...
- Tabs: **Templates** and **Sets**. Filter by desk type (All / DCA / Perps / Cash and Carry).
- **My templates:** rename, description, delete, replace recipe (re-save from a desk, or “update from playbook” later if we add a picker). Create a set from checkboxes (same `desk_type` only).
- **Platform:** read-only cards. **Apply** opens a desk picker (matching type). Members cannot edit or delete platform rows here.
- **My sets:** name, description, order items, add/remove (own templates and platform templates), delete, Apply (desk picker).
...
2. **User** — every member’s templates and sets. Columns: name, desk type, owner email, updated. View recipe preview (no secrets in JSON). Rename, description, delete (confirm: owner loses it). **Publish copy to platform** (new platform row; user row unchanged). Cannot apply a user template onto **that member’s** desks from here (no impersonation). An admin applies only to **their own** desks, from Automations or `/account/templates`.
...
| See another member’s user templates on `/account/templates` | No | No |
| Apply my or platform template/set to **my** desk | Yes | Yes |
...
- Sharing user templates with another login (platform copy is the share path)
...
- Marketplace, import/export files (can add later as JSON download of `recipe`)
