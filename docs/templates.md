...
- **Export all** / **Import** on the right of the My Templates / My Folders toolbar, just left of **Show Filters**. Bulk actions sit on the left of that row when rows are selected. Import is a modal: pick a JSON file, then choose templates and folders. Shared tabs keep row **Import** / **Remove** only.
