...
**Bot form (draft)** is the **Bot form** tab on this page (`/admin/theme?tab=bot`). Proposed standard for every desk. Optional sections use a styled checkbox to show or hide settings (no Off in the method dropdown). Same section order. Same chrome: no Actions column. **Save** sits at the top of the bot card only when that bot is dirty, with “You have unsaved changes on this bot.” **Status** is a dropdown; Save applies the selected mode. **Active** + Save turns the bot on (today’s Save and Arm). **Disabled** + Save closes that bot’s positions and turns it off (today’s Close bot). A note under Status says what Save will do. Sample desks: Perps, DCA, and C&C. Each desk keeps its own extra status (Perps / C&C Reduce only, DCA Stop adding). **Active** and **Disabled** are the same words on both. Those lists do not merge. Hints sit on the label or section title itself (hover), not an i icon. When an optional section is on, its required fields must be filled before Save. Trigger settings title is always **Trigger - {kind}** (for example Trigger - Price Cross). Perps **Skip if this side is already open** sits in What & When (entry rule, not an exit). Trigger settings and Secondary Condition / Exit-if use a 5-column row when the fields fit. Footer actions sit under **Additional Actions** (Backtest, Save as template, Save as platform template, Remove). DCA remaining adds use the same Market / Limit pill as Perps Order, not a GTC checkbox. Local mock only. Do not apply to live Perps / DCA / backtest forms until Click signs it. The **Theme** tab keeps the colour, type, and control reference.
