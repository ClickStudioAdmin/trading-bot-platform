...
The scale is on **Theme → Cards** (`/admin/theme?tab=cards`), with what is still inconsistent. Live card titles are 18px. Profile and password field labels are 14px muted, matching the bot form. Other field labels are still white or 12px. Helpers under Profile fields are still 13px faint.
...
| Card intro / form label | 14px muted | `text-sm text-ink-muted` | Sentence under a title; bot-form field labels |
