...
**Bot form (draft)** is the **Bot form** tab on this page (`/admin/theme?tab=bot`). Proposed standard for every desk (Off, not a checkbox that hides exits; same section order). The main card is one armed bot as it would look live (Name + **Status and Actions**, footer Backtest / template / Remove). DCA summary / ladder and other states sit in **Reference** under the card. Local mock only. Do not apply to live Perps / DCA / backtest forms until Click signs it. The **Theme** tab keeps the colour, type, and control reference.
