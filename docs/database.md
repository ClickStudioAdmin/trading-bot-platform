...
| `paper_carries` | 3 | Blotter. Scoped by `account_id`. RLS by `user_id`. Authenticated select/insert/update. No delete. Multiple open rows per pair are allowed. `source` is how it opened (`manual` or `engine`). `status` is `open`, `closing`, or `closed`. `close_source` is how it closed. `close_reason` is the auto-exit or `unwind`. Engine rows also store the entry snapshot and per-trade exits. On a Connected Exchange book these rows are venue fills (Phase 7). |
