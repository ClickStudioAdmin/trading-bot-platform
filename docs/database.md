...
| `strategy_settings` | 8 | Per-account, per-strategy bind. Phase 8 uses `strategy_id = futures` for the Futures exchange bind, reduce-only flag, and optional Buy/Sell risk caps (`max_qty_per_symbol`, `max_notional_per_symbol`, `max_open_rows`). Cash-and-carry stays on `paper_engine_settings`. RLS by `user_id`. Service-role writes. Authenticated select. |
