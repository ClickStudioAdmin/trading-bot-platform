...
| `futures_working_orders` | 8 | Resting Futures GTC limits. Status may update (`open` / `filled` / `cancelled` / `rejected`). Optional TP/SL and trailing stop copy onto the position on fill, including partial TP/SL qty. `reduce_only` close limits reduce or flatten the bound open row instead of opening. Fills still append to `futures_orders`. RLS by `user_id`. Authenticated select/insert/update. No delete. |
