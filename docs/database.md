...
| `paper_engine_settings` | 4 | Per-account engine flags and usable book share. Primary key is `account_id`. Account `reduce_only` stops new automated entries and scale-ins across Active sets; exits, clips, Unwind, and manual Open/Close still run. |
