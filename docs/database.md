...
| `futures_working_orders` | 8 | Resting Futures GTC limits. Status may update (`open` / `filled` / `cancelled` / `rejected`). Fills still append to `futures_orders`. RLS by `user_id`. Authenticated select/insert/update. No delete. |
