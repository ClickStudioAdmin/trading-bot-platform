...
| `futures_positions` | 8 | Single-leg Futures blotter. One open row per `(account_id, symbol, side)`. Buy adds to a long, Sell adds to a short, Close closes that side. Optional `take_profit` / `stop_loss` with Last/Mark/Index triggers. RLS by `user_id`. Authenticated select/insert/update. No delete. |
...
| `futures_working_orders` | 8 | Resting Futures GTC limits. Status may update (`open` / `filled` / `cancelled` / `rejected`). Optional TP/SL copies onto the position on fill. Fills still append to `futures_orders`. RLS by `user_id`. Authenticated select/insert/update. No delete. |
