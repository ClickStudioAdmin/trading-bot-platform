...
| What copies | **Fills** and **resting limits** (DCA GTC clips and exit limits), sized the same way. Recipes, bots, and keys stay on the parent. A parent limit is copied as a limit at the same price. Cancel and amend follow the parent. When that parent limit fills, the copy limit is cancelled and the fill is market-copied, unless the copy limit already filled — then the parent fill is receipted and not placed again. **Every desk type is one trade per symbol + side.** If the parent already has that position (or add limits on it) and the copy is not already in it, the copy takes nothing — no adds, no new limits, no TP/SL — until the parent is flat and opens a new trade. If the copy is already in that trade, it keeps following amends, cancels, reduces, and flatten. **DCA is all-or-nothing on that ladder.** If the follower cannot copy the whole ladder, it copies none and waits for the next cycle. |
| Sizing | **Same formula, three books.** Follower order USDT = parent fill USDT × (follower book / parent available). Parent $100k opens $10k is always 10% of the follower book. **Account balance** (default): book = real available. **Percent of account:** book = available × N%. **Fixed book:** book = dummy $X; if real available drops below $X, skip the fill and pause copying. Live available is the bound key; if available is 0, fan-out uses margin balance so a desk already in trades still copies. Paper book is **10,000 USDT + realized + unrealized**; the follow modal and Desk Settings name that start, and Market Data hover on paper desks shows live paper equity. Read when the fill copies. Skip if the parent fill, parent book, or resolved follower book is not a positive number. Paper copy sizes below the venue lot (so a 1/10 book still shows the parent multiplier). Paper flatten and row Close use that same paper qty step — venue min does not block closing leftover dust. Live copy does **not** round a clip up to the exchange minimum — if any clip in the trade is below min qty or min notional, skip the whole trade. Copy desks do **not** apply max value per symbol, max open positions, or max open notional — those would fight the size book. |
| Follower UI | Own blotter. Compact header: parent trader, parent desk, Pause, and Unfollow (browser confirm; open trades or last desk still block). Bind, sizing, reduce-only, max daily loss, max drawdown %, and max adverse move % are **Desk Settings**. Paper desks use the same **Market Data** chip; hover shows paper equity (starts at 10,000 USDT). Copy is a popup (desk on the left, guards on the right). After follow, stay and close; **Go to Copy Desk** under Copy jumps to the new desk. No ticket, Automations, playbooks, webhooks, templates apply, Pairs, or backtest-from-this-desk. Positions and Performance stay desk-wide: the bot data-set dropdown and Bot filter are hidden, and a `bot` query does not narrow the book. A copy of **DCA** uses the same Positions / Open orders chrome as a normal DCA desk (Entry #, Orders, planned exits) from the parent recipe; it does not get playbook edit or arm. Row **Close** flattens that copied position at market — it is not **Close bot**. Perps / bots / TradingView copies stay on the generic perps blotter. Activity on a copy desk logs parent/child events: followed, paused, resumed, copied fills and limits, amends and cancels to match the parent, cycle skips, and per-fill skip reasons. |
...
| Stats | **Snapshotted** on every futures desk (shared or not; not C&C). All-time and 30d realized P&L, win count, and realized giveback (peak-to-trough of cumulative realized, close-time order). Catalogue 30d **Drawdown** is that giveback in dollars (parent desks are live). Desk / copy-details **Max Drawdown**: paper books use peak-to-trough of the paper book (10,000 USDT start + realized) versus that peak; live books show two closed-book dollars — realized giveback and max realized loss (largest losing trade). Not venue-wallet equity and not other desks on a shared key. Trader stats: unique followers, visible desk count, first shared. No rolled-up P&L across desks. No AUM, profit share, Sharpe, or views. |
| Engine | Fly worker / tick. Parent desk tick after reconcile (and after bots on that desk). Fills market-copy on the follower. Open working orders sync (place / amend / cancel). When the parent is flat on a symbol and side (no position, no entry limits) and the follower still has that position, the follower flattens — closes do not depend on reading the parent book, and paper flatten does not apply venue min qty. Every copied desk gates **entries** per symbol + side: mid-flight skip if the parent position opened before follow (or clip index > 0), or live size below venue min. Writes `copy.cycle_skipped` once and receipts that trade’s entry fills (not flatten). Do not copy reduce-only / TP/SL if the copy never joined. Drawdown uses follower equity (available plus unrealized) against a stored peak. Adverse-move skip writes a receipt. Pause skips new entries; closes still copy. Idempotency `desk_copy_receipts` plus command key = parent fill id; copied DCA limits keep the parent clip key so Entry # parses. One desk lease. Browser never sees parent keys or recipes. Paper equity starts at 10,000 USDT plus realized and unrealized. |
...
Site header **Copy Trading** (`/account/copy`) is one catalogue: public listings plus the viewer’s open private invites. **Private** badge and a private-only filter. **Favorites** are desk bookmarks. **Subscribed** is desks the viewer is currently following (an active copy desk of that parent). Catalogue never shows email. Parent desks keep **Manage Copy Traders**. Sidebar shows a **Copy** badge on follower desks. Type grouping stays Automated vs Manual. Trader alias opens the trader page (desks visible to the viewer). **View details** opens `/account/copy/desks/[accountId]` — the owner’s closed-book Performance page plus a header (desk, followers, trader) with Copy / Following in the top-right of that card, and the same all-time tiles as desk Performance (days trading, completed trades, win rate, max drawdown, realized profit, P&L, ROE on margin, APR as annualized P&L). Catalogue and header **Copy** buttons use the Copy Trading (Users) icon. Past Positions on that public page has Pair and Side filters (no Bot — recipes stay on the parent). Header tiles stay the full closed book. Paper P&L is realized ÷ $10,000 start; live P&L is realized ÷ peak margin used. Parent details are live, so Max Drawdown shows realized giveback and max realized loss in dollars. A paper copy follower’s own Performance page keeps the single book-equity drawdown. Invited is not a tile. That page stays visible if the viewer already follows, even after sharing is later turned off. Catalogue cards show 30d **P&L** (on notional) as the hero, then realized $, win rate, drawdown $ (realized giveback), and followers. Invited is not on the card. Sort **30d P&L** still uses the `roi` query key. Listing has a catalogue **Desk name**. Copy is a popup that stamps type/venue from the parent; follower picks Paper or Live, a sizing book, and optional max daily loss, max drawdown %, and max adverse move %. Reduce-only stays in Desk Settings. `/account/copy/desks/new` redirects to the catalogue with the popup open.
