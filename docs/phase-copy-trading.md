...
| Sizing | **Same formula, three books.** Follower order USDT = parent fill USDT × (follower book / parent available). Parent $100k opens $10k is always 10% of the follower book. **Account balance** (default): book = real available. **Percent of account:** book = available × N%. **Fixed book:** book = dummy $X; if real available drops below $X, skip the fill and pause copying. Live available is the bound key. Paper book is **10,000 USDT + realized + unrealized**; the follow modal and Desk Settings name that start, and Market Data hover on paper desks shows live paper equity. Read when the fill copies. Skip if the parent fill, parent available, or resolved book is not a positive number. Copy desks do **not** apply max value per symbol, max open positions, or max open notional — those would fight the size book. |
