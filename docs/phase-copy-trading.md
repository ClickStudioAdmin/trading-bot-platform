...
| Engine | Fly worker / tick. Parent desk tick after reconcile (and after bots on that desk). Fills market-copy on the follower. Open working orders sync (place / amend / cancel). When the parent is flat on a symbol and side (no position, no entry limits) and the follower still has that position, the follower flattens — closes do not depend on reading the parent book. Every copied desk gates **entries** per symbol + side: mid-flight skip if the parent position opened before follow (or clip index > 0), or live size below venue min. Writes `copy.cycle_skipped` once and receipts that trade’s entry fills (not flatten). Do not copy reduce-only / TP/SL if the copy never joined. Drawdown uses follower equity (available plus unrealized) against a stored peak. Adverse-move skip writes a receipt. Pause skips new entries; closes still copy. Idempotency `desk_copy_receipts` plus command key = parent fill id; copied DCA limits keep the parent clip key so Entry # parses. One desk lease. Browser never sees parent keys or recipes. Paper equity starts at 10,000 USDT plus realized and unrealized. |
