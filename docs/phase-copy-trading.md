...
- Follower settings: unused `scale` (create writes `1`), `size_mode` (`balance` / `percent` / `fixed`) plus `size_percent` or `size_book_usdt`, pause, max daily loss / max open notional (caps stay on `strategy_settings`). Unfollow revokes the grant and deletes the copy desk when it is flat and not the last desk.
