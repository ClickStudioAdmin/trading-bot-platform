...
Account nav gains **Copy desks** (catalogue). Parent desks get a **Manage Copy Traders** tab (follower list + private email invite; alias only, never email). Sidebar shows a **Copy** badge on follower desks. Type grouping stays Automated vs Manual.
...
| 5 | Private grants | Agent | Email invite from **Manage Copy Traders**. Follower list shows trader alias only (never email). Revoke, unlist, and unbind pause new entries. **In repo 31 Aug 2026.** |
