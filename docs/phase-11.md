...
First managing playbook. The app owns clips and exits. Signals only arm. Same perp blotter and `runFuturesCommand`. Stacked playbooks on the desk (one per contract). Direction can be long, short, or both. Cannot stack scale-in or Perps recipes on that desk.
...
9. The tick places clips and exits through `runFuturesCommand`. Opened rows show Auto plus the playbook name, plus Clip / Next add / Remaining.
