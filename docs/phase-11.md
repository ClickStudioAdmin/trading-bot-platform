...
9. The tick places orders and exits through `runFuturesCommand`. Opened rows show Auto plus the playbook name, plus Orders as filled/max. Open orders show Side (Buy/Sell) and Type (Entry #, Take Profit, Stop Loss, or Close). Positions and Overview refresh the blotter while the tab is visible so fills and resting GTCs show without a reload.
