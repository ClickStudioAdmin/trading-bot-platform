...
Complete. Accepted 27 Aug 2026. Click will keep desk-testing; treat the phase as done. `dca_playbooks` migrates on push to `develop`. If Automations errors on `breakeven_activation_pct`, push `20260827140000_dca_playbooks_advanced.sql`. If it errors on `take_profit_order_type`, push `20260827170000_dca_exit_order_types.sql`. If Save rejects a 30m–daily indicator timeframe, push `20260828080000_dca_indicator_timeframes.sql`. If it rejects RSI/EMA/MACD crosses (`cross_gte` / `cross_lte`), push `20260828090000_dca_indicator_crosses.sql`. If Save rejects `% of account` max value, push `20260901160000_dca_max_value_kind.sql`. If Save rejects `% of available margin`, push `20260902210000_dca_max_value_margin.sql`. If Save rejects EMA/SMA period, push `20260908120000_dca_indicator_ma.sql`. If Short MACD When still looks flipped, push `20260908140000_dca_indicator_when_any_side.sql`. If Save rejects Bollinger Bands, push `20260908150000_dca_indicator_bb.sql`. If Save rejects a MACD level of 0 or a negative histogram level, push `20260908160000_dca_indicator_macd_level.sql`. If Save rejects EMA Cross / SMA Cross periods, push `20260908170000_dca_indicator_ma_cross.sql`. If Save rejects Trend / Supertrend, push `20260909080000_dca_start_trend_supertrend.sql`. If Save rejects ATR spacing or ATR take profit, push `20260909090000_dca_atr_spacing_tp.sql`. If Save rejects Confirm or Exit-if, push `20260909120000_dca_confirm_exit_if.sql`.
...
| 4 | Signals | Agent | Manual arm. Bound Signal `arm` / `disarm` / `close-playbook`. Buy / sell arms that side only. Price, indicator, Trend, or webhook start. Done |
...
3. Direction is long, short, or both. Both opens two blotter rows that add independently and never flatten each other. Start is automated only (price, indicator, Trend, or Signal). Save and Arm listens; the first order waits for that condition.
4. Start is price cross, a bound Signal webhook, an indicator on public klines (RSI, MACD, Price vs SMA, Price vs EMA, SMA Cross, EMA Cross, Price vs BB; 5m through daily), or **Trend** (Supertrend only: ATR period default 10, multiplier default 3; When is turns/is bullish or bearish). Supertrend is not in the Indicator dropdown. RSI can sit at a level or **cross** it (period, default 14). When is the real condition on either side: Long or Short can pick any RSI cross/sit, any MACD histogram cross/sit vs a typed level (default 0), **EMA Cross / SMA Cross** (fast period, crosses above/below, slow period; default 9 / 21), **price crosses or is above/below** EMA/SMA (period, default 21), or **price crosses / is above top BB or below bottom BB** (period, default 20; 2 standard deviations). Saved bots that still use EMA 21 vs a typed price keep working. Direction **Both** uses two trigger sections (**Long** / **Short**): each side has its start and its own Confirm. Start is that side’s price, or its own indicator / When / timeframe / level / period, or its own Supertrend period / multiplier / When / timeframe. Long and Short never fire on the same print. Optional **Confirm** (Off by default) is one AND filter under that side’s start: Price vs EMA / SMA, RSI, Price vs BB, ATR band, or Supertrend. Confirm / Exit-if When is a state (is above, is bullish), not a cross or turn. Confirm is now-only (no latch); missing bars are not met. Cycle-lock Confirm with Start. Idle uses Save and Save and Arm. Live uses Save; Arm resumes after Stop adding. Close returns to idle. Price/indicator/Trend then wait for the condition. Webhook: the bound Signal’s `arm` / buy / sell places the first order only after Arm. Idle recipes ignore the Signal.
5. Adds: **Maximum Exposure** first (max orders and/or max value). While a side has an open blotter position, cycle fields (pair, direction, start, Confirm, size, caps, averaging, multipliers) are locked. Take profit, stop, Exit-if, trailing, and breakeven still save and sync resting exits. Armed with no position stays fully editable. Leftover fill counts after a flatten do not lock the form. Max value is **No max value**, a **fixed USDT amount**, a **% of account**, or a **% of available margin**. Paper book is $10,000 + realized; live book is available on the bound key. Fixed and both percent kinds require an amount. **100% is the cap** on both percent fields. `% of account` is book × pct (notional). `% of available margin` is book × leverage × pct, so 100% is full buying power. Both resolve to dollars at the **start of each cycle** (first clip) and stamp so the ladder does not resize mid-cycle — that is auto-compounding. When both caps are set, the first clip is calculated from the resolved max value, max orders, and the size multiplier (USDT exact; qty needs last price). Size-first still works if only one cap is set. Then one averaging kind — add on price deviation or add on interval (minutes, hours, or days). Price deviation can place remaining orders as GTC limits instead of market. Those remaining GTCs still sync from the locked recipe and the tick: extras cancel, and Close All does not count as filling the cap. Switching to GTC rests remaining orders with one key per entry index **and this position** so a prior cycle’s filled Entry # 2 or # 14 cannot block the next rest. Save and the tick share that key. Live desks rest a few per tick, lowest Entry # first, and cancel extras at the same index; later ticks finish the ladder so Bybit is not flooded in one 60s engine run. Size and deviation multipliers scale later orders. Caps stop adding. They do not flatten. Save (and Arm) is blocked in the form when any order is above the venue max qty, with an amber warning, unless a position is already open (then only exits save). Stop adding and Close playbook still run if the recipe is already over that cap.
6. Exit: take profit / stop vs average or first fill. Take profit can fill Market (default) or as a GTC limit. Stop is market only. Optional **Exit-if** on Stop loss is its own Off filter (same catalog as Confirm, not the inverse). Market flatten of that side; editable while open. Tick order is % SL, then Exit-if, then TP. Close reason `exit_if`. GTC take profit is the close: the tick does not also flatten at market while that limit is resting. If the GTC is missing, cancelled, or rejected, the tick flattens at market. Save and the tick share one rest (stable key per position, extras cancelled). Cancelling that GTC drops the receipt so the next rest can place again. A leftover `working` receipt with no open GTC does not block the next rest. Limit take profit is the GTC: the tick does not call Bybit trading-stop every cycle just to keep that limit on the position row. After a partial fill, a larger add cancels the leftover and rests a new GTC for the full remaining qty instead of amending a reduce-only order up. Place/amend rejects write `dca.sync_failed`. First market order, flatten, and TP rest use stable keys so a retry does not double-place. Optional move-stop-to-breakeven and trailing on the existing row trailing engine. After adds, trailing distance refreshes from mark and only tightens. Stop loss never moves away from price (long: never lower; short: never higher). Positions show recipe prices faint until a limit is resting, then TP green / SL red. Saving a live playbook rests or cancels the TP GTC. Stop adding is only offered while a side is open; it leaves the position. Armed with no position can Disarm (idle). Close bot on Automations flattens then returns to idle. Save is hidden until the form differs from the saved bot.
7. No Buy/Sell ticket. Each open row’s Close By is **Close** (flatten that side, cancel its working bot orders, stay armed). **Close bot** on Automations still flattens and idles. Positions panic is **Close All & Cancel All Open Orders** only (does not idle the playbook). After the position is gone, price / indicator / Trend / Signal stay armed and wait for the next start. Reduce-only, caps, and playbook TP/SL still protect. Positions show those levels read-only; faint is the recipe or a market exit, colour means a limit is resting. No Add/Edit on the row. No Market/Limit qty close, positions-only Close All, Cancel All Open Orders, or row Edit/Cancel on working limits. Open orders hide TP/SL and Trailing: those attach on the position, not each GTC. Reduce-only blocks new orders; TP/SL still run.
