...
| 2 | Playbook store | Agent | One DCA config per desk (not a list of stacked playbooks). Fields: clip size, max clips or max value, add-on-dip and/or add-on-interval, take profit / stop, “stop adding”. Done |
...
| 5 | UI | Agent | Create Desk → DCA. Type-locked chrome (no ticket, no Perps recipes, Signal arm door). Automations is the playbook form. Positions show Clip / Next add / Remaining. Done |
...
6. Signals arm, disarm (stop adding), or close the playbook. TradingView **Order** webhooks are not the brain on this desk. Signal webhooks are. Optional price-cross on the playbook form can arm or disarm.
