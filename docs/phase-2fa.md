...
Do not implement until Click starts this item. Stop when the criteria below pass.
...
A member can turn on TOTP on Account Settings. After that, sign-in asks for the code. That is the whole item. A plan **requiring** 2FA before Live / copy / backtest is **not** this item.

## Done when

1. Account Settings → Security (or Password tab) can enable TOTP. Recovery codes shown **once**. Secrets encrypted at rest with the same class of key as other credentials. Recovery codes are hashed.
