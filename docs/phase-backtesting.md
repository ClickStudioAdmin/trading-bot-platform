...
Same `<DeskChart>`, candle API, and overlay renderer. Chart toolbar includes a TradingView-style full-screen control (Escape to leave). Pinned and fitted views keep empty bars on both edges so Entry / Add / TP labels are not clipped, and overlay price lines use short TP / SL / Liq titles. Backtest overlay source = `backtest_runs.orders` plus the When line (Perps only). **All trades** draws every Entry and Exit; 1h and finer also draw Adds. Pinning a cycle (chart Prev/Next or a positions row) adds that cycle’s Adds and planned entry / TP / SL / Liquidation lines, and zooms the time scale to that trade. Completed fills use Entry / Add / TP / SL / Trail / Close / **Liq**. Still-open clips use **Open long/short** (circle). Fills sit on the **containing** display candle (bar open), not the nearest midnight — an afternoon 15m fill stays on that day’s Daily bar. Past Positions tags a wiped row **Liq**, and shows Side and Value instead of Source. Header and Performance stay realized only.
