...
Same `<DeskChart>`, candle API, and overlay renderer. Backtest overlay source = `backtest_runs.orders` plus planned entry / TP / SL lines (from the open cycle, or the last closed cycle) and the When line (Perps only). Every fill is on the chart. Completed fills use Entry / Add / TP / SL / Trail / Close / **Liq**. Still-open clips use **Open long/short** (circle). Header and Performance stay realized only.
