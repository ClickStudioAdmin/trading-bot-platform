...
Same `<DeskChart>`, candle API, and overlay renderer. Backtest overlay source = `backtest_runs.orders` plus planned entry / TP / SL lines (from the open cycle, or the newest closed cycle) and the When line (Perps only). Completed fills use Entry / Add / TP / SL / Trail / Close / **Liq**. Still-open clips use **Open long/short** (circle). On a long tape, earlier cycles keep only the exit marker so the **latest** cycle’s adds stay readable; a liquidation hides TP/SL and draws a Liquidation price line. Fills after the last display candle snap onto that bar. Past Positions tags a wiped row **Liq**. Header and Performance stay realized only.
