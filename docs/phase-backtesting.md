...
Same `<DeskChart>`, candle API, and overlay renderer. Backtest overlay source = `backtest_runs.orders` plus planned entry / TP / SL lines (from the open cycle, or the newest closed cycle) and the When line (Perps only). Completed fills use Entry / Add / TP / SL / Trail / Close / **Liq**. Still-open clips use **Open long/short** (circle). On a long tape, earlier cycles keep only the exit marker so the **latest** cycle’s adds stay readable; a liquidation hides TP/SL and draws a Liquidation price line. Fills sit on the **containing** display candle (bar open), not the nearest midnight — an afternoon 15m fill stays on that day’s Daily bar. Past Positions tags a wiped row **Liq**, and shows Side and Value instead of Source. Header and Performance stay realized only.
