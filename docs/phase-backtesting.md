...
| Detail | `/account/backtests/[runId]`. Parameters, stats, equity chart, inline chart, paged completed trades. Open positions at the window end are noted as current trades and omitted from the list. Queued, running, failed, or cancelled runs keep Parameters and header dates; Performance, equity, chart, and trades show a waiting or failed message — not an empty chart or “no fills”. |
