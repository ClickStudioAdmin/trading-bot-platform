...
| DCA start | Replay treats legs as **armed** at the window start (Save and Arm). Immediate fires the first clip on the first close. **Both** never starts long and short on the same print, and does not open the other side while one position is still on. A flatten on a bar also blocks a **new** start on that same bar (the other side waits for the next close). Immediate + Both takes Long first (Manual does not auto-hedge). Direction **Both** has two start sections (Long start / Short start): two prices, two indicator When / level / timeframe rows, or two Supertrend period / multiplier / When / timeframe rows. RSI 30 long does not imply RSI 30 short — short is its own level (default 70, crosses above). Legacy Both rows with one shared level are backfilled that way. |
