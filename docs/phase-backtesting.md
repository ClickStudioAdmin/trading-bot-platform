...
Same `<DeskChart>`, candle API, and overlay renderer. Chart toolbar has **Fill browser** (page) and **Full screen** (monitor via the Fullscreen API). Escape leaves monitor first, then browser fill. Pinned and fitted views keep empty bars on both edges so Entry / Add / TP labels are not clipped, and overlay price lines use short TP / SL / Liq titles. Backtest overlay source = `backtest_runs.orders` plus the When line (Perps only). **All** fits the full replay window and draws every fill (Entry, Add, Exit). Pinning a cycle (chart Prev/Next or a positions row) keeps that cycle’s Adds and planned entry / TP / SL / Liquidation lines, and zooms the time scale to that trade. Completed fills use Entry / Add / **TP long** / **TP short** / SL / Trail / Close / **Liq**. Take-profit markers stay green; the label names the side. Still-open clips use **Open long/short** (circle); remaining Adds are **Add N long** / **Add N short**, green long and red short. Closed Adds stay **Add N** with arrows. The pinned TP price line stays **TP**. Fills sit on the **containing** display candle (bar open), not the nearest midnight — an afternoon 15m fill stays on that day’s Daily bar. Past Positions tags a wiped row **Liq**, and shows Side and Value instead of Source. Header and Performance stay realized only.
