...
| Worker | Short jobs (≤3000 bars and ≤4 pairs) run in the request. Longer jobs stay `queued`. Each engine cycle drains one modest job first (≤3000 bars), then desks, then another job (Fly: any length; Vercel: ≤3000). Opening a queued run also claims and executes that row. Stale `running` after 15 minutes is reclaimable. |
