...
| Attach / Save | After **done**, header groups stay separate. **Library:** if they loaded a template and replay fields still match, **Save to {name}** links the run. Otherwise **Save to library** (new user template, run linked). Never auto-attach after an edit. **Desk:** **Add to desk** copies idle onto a chosen desk; never arm from a run. **Share:** **Publish snapshot** copies a platform `backtested` row (`user_id` null). |
