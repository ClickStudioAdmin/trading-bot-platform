...
| 4 | Manual desk test | Click | Bybit Demo: Buy, Sell, Market Close (full and a slice), Limit Close, a Limit that rests, Edit, and Cancel, plus TP/SL and trailing stop on an order and on an open row, plus Cancel & close all. Paper book writes the ledger only. |
...
8. Positions has **Cancel & close all**. Confirm by typing `CLOSE ALL`. It cancels every open working order on this book, then market-closes every open row at full size on the same path as Market close. Stops on the first error. The parent command may carry an idempotency key; child cancels and closes do not reuse it.
...
- **Cancel & close all** on Positions: cancel every working order, then market-close every open row. Confirm `CLOSE ALL`.
