...
| 4 | Manual desk test | Click | Bybit Demo: Buy, Sell, Market Close, Limit Close, a Limit that rests, Edit, and Cancel, plus TP/SL and trailing stop on an order and on an open row. Paper book writes the ledger only. |
...
- Buy/Sell Market or GTC Limit. Close is Market or reduce-only GTC Limit. Open orders table + Edit remaining qty/limit + Cancel.
...
- Typed `runFuturesCommand` for place / close / TP-SL / trailing / amend / cancel. Form actions are adapters. Optional idempotency key (Bybit `orderLinkId` on live). No webhook this phase.
