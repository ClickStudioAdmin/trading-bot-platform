...
| 4 | Manual desk test | Click | Bybit Demo: Buy, Sell, Market Close (full and a slice), Limit Close, a Limit that rests, Edit, and Cancel, plus TP/SL and trailing stop on an order and on an open row. Paper book writes the ledger only. |
