...
| 4 | Manual desk test | Click | Bybit Demo: Buy, Sell, Close, a Limit that rests, Edit, and Cancel, plus TP/SL and trailing stop on an order and on an open row. Paper book writes the ledger only. |
...
- Buy/Sell Market or GTC Limit. Close stays market. Open orders table + Edit remaining qty/limit + Cancel.
