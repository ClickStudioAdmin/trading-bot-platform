...
The tick is `runEngineCycle` in `lib/engine`: one modest queued backtest first, then shared scan, then **hot desks first** (open books, armed DCA, active Perps), then other idle desks via **per-desk leases**, then another backtest if time remains. Fly.io in **Sydney** (`tbp-engine-dev` / `tbp-engine`) loops about every 20 seconds, or 8 seconds while an indicator-start DCA bot is armed. Desk mark / P&L also poll Bybit public tickers through `GET /api/market/tickers`. See [phase-fly.md](phase-fly.md).
