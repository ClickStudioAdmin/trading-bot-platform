...
2. Desk **Automations (bots)** for DCA, Perps bots, and Cash and Carry is a Theme table: name, pair / side, a short recipe summary, status, and actions (Edit, plus today’s Disable / Close / Clone / Remove where that desk already has them). The name is a link to View/Edit (`?edit=`). Positions (open count) and Performance (ROE) sit left of Actions; each value has an icon to that desk page with `bot` set. Toolbar: **Create New Bot** (and Create from Template / Clone when that desk already has them). Empty state uses the same create control. Desk **Positions** and **Performance** filter by bot (and pair / side) so a member can see one bot’s rows and stats. A top-right data-set dropdown and the table Bot field share `bot`. Empty bot is Desk Wide (all bots). The table filter bar stays hidden until Show Filters — changing the data-set dropdown does not open it. Close All stays desk-wide.
