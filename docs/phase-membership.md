...
One active **collection method** per member (`stripe` = Card or `wallet` = Crypto). Switching to Crypto sets Stripe `cancel_at_period_end`. Switching Crypto → Card on Billing Overview shows the Stripe subscription embed under the method form (trial to `period_end` when the cycle is still open, so this month is not charged again). Method saves only after Stripe completes. An existing Stripe sub that was set to cancel at period end is resumed with a card setup embed.
