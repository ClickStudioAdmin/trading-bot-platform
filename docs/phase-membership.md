...
That write is still a **paid** invoice for the plan price (not `comp`). Upline L1–L5 still earn on it. The paying member does not earn on their own invoice. **Deduct payments from Your Wallets where possible** means “use Main first” (the USD credit book). Checkout **Pay with credit** runs that deduct now. The engine issues an **open** renewal invoice **7 days** before `period_end` (Card and Crypto). Due date is `period_end` plus a **6 hour** buffer. Crypto: collect only once the invoice is **due** (`due_at`), if Main (plus payable Affiliate if that tick is on) covers it — including right after a deposit credit on or after the due date. Do not debit the account during the 7-day lead window. Card: Stripe `invoice.paid` marks the matching open invoice paid. First checkout / upgrade still writes a paid invoice immediately. PDF invoices later.
