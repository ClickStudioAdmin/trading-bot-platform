...
**Leftover Main** stays on cancel / Free. Spend later or, when Crypto is the saved method, withdraw as USDT from the **Wallet** tab if ≥ the **Main Wallet minimum** (admin setting on Settings → Crypto, default **$100**, separate from the affiliate $50 min) and no arrears. Each request needs a receive address typed in — not the affiliate payout address. The withdraw debits Main immediately. Click generates a payout list on `/admin/billing` **Wallet withdrawal requests**, sends USDT by hand, then marks the file paid. Affiliate withdraw is a separate queue on `/admin/affiliates` (Affiliate book). Not forfeited.

**Billing page** `/account/billing` (account chrome, not the header browse row): Overview is Current plan · Payment method only. **Card** tab (Credit Card saved only) embeds Stripe setup checkout to update the card on TBP — not the hosted Customer Portal. **Wallet** tab (Crypto saved only) has Wallet balance, Main USDT withdraw (address typed each request), and Top up Main Wallet. **Account ledger** is the running Main book (deposits, plan payments, withdrawals, Affiliate → Main transfers). Invoices. Payable affiliate earnings stay on Affiliates. Plans **Upgrade** opens `/account/billing/checkout?plan=` — method on the left; Stripe **embedded** form or Pay with credit on the right. Checkout keeps Top up Main Wallet when Crypto is saved. Switching Card → Crypto requires **Save method** before the Wallet tab, deposit address, and Pay with credit unlock. Members stay on TBP. Success returns to Billing. Admin `/admin/billing` tabs: Overview (deposit rails), **Invoices** (every member invoice), Wallet withdrawal requests.
