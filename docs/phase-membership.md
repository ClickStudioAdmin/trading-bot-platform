...
| --- | --- | --- |
| **Account Wallet** (ledger book `main`) | Unused USD credit from 1:1 stablecoin top-ups (and any transfer in from Affiliate) | `deposit`, `transfer_in`, `debit_rent`, `withdraw`, `adjust` |
...
**Leftover Main** stays on cancel / Free. Spend later or withdraw as USDT from the **Manage Wallet** tab if ≥ the **Main Wallet minimum** (admin setting on Settings → Crypto, default **$100**, separate from the affiliate $50 min) and no arrears. The Manage Wallet tab stays while Crypto is the saved method, or while Main still has leftover after a switch to Card, until the balance is zero. Each request needs a receive address typed in — not the affiliate payout address. The withdraw debits Main immediately. Click generates a payout list on `/admin/billing` **Wallet withdrawal requests**, sends USDT by hand, then marks the file paid. Affiliate withdraw is a separate queue on `/admin/affiliates` (Affiliate book). Not forfeited.

**Billing page** `/account/billing` (account chrome, not the header browse row): Overview is **Current subscription** (current plan, monthly payment, billing cycle, next payment due), plus Payment method after they have picked Card or Crypto at checkout (hidden on a new Free login with status none). **Card** tab only when Credit Card is the saved method and the current plan is paid (hidden on Free). It embeds Stripe setup checkout to update the card on TBP — not the hosted Customer Portal. **Manage Wallet** tab shows when Crypto is saved, or while Main still has leftover after switching to Card, until the balance is zero. It has Wallet balance, Account Wallet USDT withdraw (address typed each request), and Top up Account Wallet (deposit address only when Crypto is the saved method). **Wallet Ledger** is last in the tab row (after Manage Wallet): the running Account Wallet book (deposits, plan payments, withdrawals, Affiliate → Account Wallet transfers). Hidden on Free unless Main already has rows (downgrade leftover). Invoices. Payable affiliate earnings stay on Affiliates. Plans **Upgrade** opens `/account/billing/checkout?plan=`. Summary card is **Subscription Details**: **Current Plan**, **Upgrading to**, **Due Today**, **Then**, plus **Payment Method** when the method is already saved. First paid checkout (Free or subscription none): Card / Crypto picker plus Stripe **embedded** form or Crypto deposit. Method saves as they switch (no Save method). Crypto shows amount due, a note to send extra for later months, and the deposit address — no wallet balance. The page polls for deposits every 10s and pays the plan when enough credit confirms. Deduct from Affiliate if required is **on** by default. Paid upgrade: no method picker; due-today is the prorate. Crypto upgrade shows Wallet balance and **Pay with credit** when Main covers Due Today; otherwise a short notice, the deposit address, and the same 10s deposit spinner (no Pay button until they have enough). Switching Card → Crypto on Billing requires **Save method** before the deposit address and Pay with credit unlock. Members stay on TBP. Success returns to Billing. Admin `/admin/billing` tabs: Overview (deposit rails), **Invoices** (every member invoice), Wallet withdrawal requests.
