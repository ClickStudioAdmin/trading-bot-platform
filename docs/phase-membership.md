...
**Leftover Main** stays on cancel / Free. Spend later or withdraw as USDT if ≥ minimum and no arrears — same request path as affiliate payouts (saved chain/address on Affiliates → Settings). The withdraw debits Main immediately. Click generates a payout list on `/admin/billing` **Wallet withdrawal requests**, sends USDT by hand, then marks the file paid. Affiliate withdraw is a separate queue on `/admin/affiliates` (Affiliate book). Not forfeited.
