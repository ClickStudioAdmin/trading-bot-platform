...
**Billing page** `/account/billing` (account chrome, not the header browse row): Overview is **Current subscription** (current plan, monthly payment, billing cycle, next payment due), plus Payment method after they have picked Card or Crypto at checkout (hidden on a new Free login with status none). **Card** tab only when Credit Card is the saved method and the current plan is paid (hidden on Free). It embeds Stripe setup checkout to update the card on TBP — not the hosted Customer Portal. **Manage Wallet** tab shows when Crypto is saved, or while Main still has leftover after switching to Card, until the balance is zero. It has Wallet balance, Main USDT withdraw (address typed each request), and Top up Main Wallet (deposit address only when Crypto is the saved method). **Wallet Ledger** is last in the tab row (after Manage Wallet): the running Main book (deposits, plan payments, withdrawals, Affiliate → Main transfers). Hidden on Free unless Main already has rows (downgrade leftover). Invoices. Payable affiliate earnings stay on Affiliates. Plans **Upgrade** opens `/account/billing/checkout?plan=`. Summary card is **Subscription Details**: **Current Plan**, **Upgrading to**, **Due Today**, **Then**, plus **Payment Method** when the method is already saved. First paid checkout (Free or subscription none): Card / Crypto picker plus Stripe **embedded** form or Pay with credit. Method saves as they switch (no Save method). Crypto shows the deposit address immediately. Deduct from Affiliate if required is **on** by default. Paid upgrade: no method picker; due-today is the prorate. Checkout shows Top up Main Wallet under Wallet balance when Crypto is selected (always on first paid checkout). Switching Card → Crypto on Billing requires **Save method** before the deposit address and Pay with credit unlock. Members stay on TBP. Success returns to Billing. Admin `/admin/billing` tabs: Overview (deposit rails), **Invoices** (every member invoice), Wallet withdrawal requests.
