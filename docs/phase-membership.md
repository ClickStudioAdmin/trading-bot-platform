...
**Leftover Main** stays on cancel / Free. Spend later or withdraw as USDT from the **Wallet** tab if ≥ the **Main Wallet minimum** (admin setting on Settings → Crypto, default **$100**, separate from the affiliate $50 min) and no arrears. The Wallet tab stays while Crypto is the saved method, or while Main still has leftover after a switch to Card, until the balance is zero. Each request needs a receive address typed in — not the affiliate payout address. The withdraw debits Main immediately. Click generates a payout list on `/admin/billing` **Wallet withdrawal requests**, sends USDT by hand, then marks the file paid. Affiliate withdraw is a separate queue on `/admin/affiliates` (Affiliate book). Not forfeited.

**Billing page** `/account/billing` (account chrome, not the header browse row): Overview is Current plan · Payment method only. **Card** tab (Credit Card saved only) embeds Stripe setup checkout to update the card on TBP — not the hosted Customer Portal. **Wallet** tab shows when Crypto is saved, or while Main still has leftover after switching to Card, until the balance is zero. It has Wallet balance, Main USDT withdraw (address typed each request), and Top up Main Wallet (deposit address only when Crypto is the saved method). **Account ledger** is the running Main book (deposits, plan payments, withdrawals, Affiliate → Main transfers). Invoices. Payable affiliate earnings stay on Affiliates. Plans **Upgrade** opens `/account/billing/checkout?plan=`. First paid checkout: method on the left if unset; Stripe **embedded** form or Pay with credit on the right. Paid upgrade: no method picker; due-today is the prorate. Checkout shows Top up Main Wallet when Crypto is the method and Main is short (always on first paid checkout). Switching Card → Crypto on Billing requires **Save method** before the deposit address and Pay with credit unlock. Members stay on TBP. Success returns to Billing. Admin `/admin/billing` tabs: Overview (deposit rails), **Invoices** (every member invoice), Wallet withdrawal requests.
