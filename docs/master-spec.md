...
- A member can have many desks. Each desk is Paper or Live at create, and has a type (`cash_and_carry`, `perps`, `signal_follower`) that locks the UI. Paper uses the in-app ledger. Connected Exchange desks place venue orders from Sydney Vercel when a key is bound (Phase 7).
