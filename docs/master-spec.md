...
- New members start with zero desks. After they confirm email they land on Overview (`/account`) and can create a desk from Manage desks when they want. A new sign-in also lands on Overview, even if they already have desks. After the first desk exists, at least one must remain.
...
The plan after Phase 11 is [roadmap.md](roadmap.md) (**V1 locked order, V2 unordered**). **V1 items 1–3 are accepted** (identity, 2FA, UI refinement). Isolation (one connection per desk, one venue account per login + venue + environment) shipped during item 3 on 19 Sep 2026. **Current is account Positions / Bots / Automations lists** (item 4, [phase-account-blotter.md](phase-account-blotter.md)) — desk Automations first. Then entitlements / plan gates. Plans, payments, and affiliates are shipped ([phase-membership.md](phase-membership.md)). Notifications steps 1–8 are in repo ([phase-notifications.md](phase-notifications.md)). Standing notes: [click-list.md](click-list.md). Automation templates: [templates.md](templates.md).

## Later

Do not implement until Click starts that roadmap item. Order and notes: [roadmap.md](roadmap.md).

**Plans, payments, and affiliates** (shipped). Spec: [phase-membership.md](phase-membership.md). Closed at step 8. Identity is V1 item 1. Entitlements / plan gates are V1 item 5 ([phase-entitlements.md](phase-entitlements.md)).
...
Bring-your-own API keys, stored on the **login**. Live desks bind one key. **One connection, one desk** (enforced on create / bind / rebind). Isolation is another trade-only key on a **different venue account** (Bybit UID / Hyperliquid wallet). A second key that hits the same venue account is rejected on Check / Save / Replace. Account-wide Positions / Bots lists stay V1 item 4 ([phase-account-blotter.md](phase-account-blotter.md)). No custody of user funds. Trade-only keys, no withdrawal. The connection model is venue-agnostic; Bybit is the first enabled venue. Hyperliquid is shipped as a venue ([phase-hyperliquid.md](phase-hyperliquid.md)); other CEXes are V2. Connected Exchange books show a Unified account snapshot (available, margin, IM/MM) from the bound key on My Account and on hover of the strategy exchange chip. The active desk for a tab is `?desk=` on desk-scoped URLs. The session cookie is last-used only.
