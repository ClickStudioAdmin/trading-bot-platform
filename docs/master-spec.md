...
- A member can have many desks. Each desk is Paper or Live at create, and has a type (`cash_and_carry`, `perps`, `signal_follower` / TradingView Strategy) that locks the UI. Paper uses the in-app ledger. Connected Exchange desks place venue orders from Sydney Vercel when a key is bound (Phase 7).
...
Phase 10 — Typed desks. See [phase-10.md](phase-10.md). Phase 1 through Phase 9 are complete. Hyperliquid and additional CEX adapters wait. Paper auto-switch is postponed ([phase-auto-switch.md](phase-auto-switch.md)).
