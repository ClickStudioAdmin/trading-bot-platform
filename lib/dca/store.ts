...
function configColumns(config: DcaPlaybookConfig): Record<string, unknown> {
  return {
    ...
    spacing_kind: config.spacingKind ?? "percent",
    atr_period: config.atrPeriod ?? null,
    atr_spacing_mult: config.atrSpacingMult ?? null,
    take_profit_kind: config.takeProfitKind ?? "percent",
    take_profit_atr_mult: config.takeProfitAtrMult ?? null,
    ...
    indicator_slow_period: config.indicatorSlowPeriod,
    indicator_multiplier: config.indicatorMultiplier,
