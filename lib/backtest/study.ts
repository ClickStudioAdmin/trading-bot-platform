  ...
  if (recipe.kind === "dca") {
    const start =
      recipe.startKind === "indicator"
        ? `${recipe.indicatorKind ?? "indicator"} ${recipe.indicatorCompare ?? ""} ${recipe.indicatorLevel ?? ""}`.trim()
        : recipe.startKind === "price" && recipe.armTrigger
          ? `Price ${recipe.armTrigger.compare} ${recipe.armTrigger.price}`
          : recipe.startKind;
    return [
      { label: "Type", value: "DCA" },
      { label: "Name", value: recipe.name },
      { label: "Contract", value: recipe.symbol },
      { label: "Direction", value: recipe.direction },
      ...
      {
        label: "Take profit",
        value: recipe.takeProfitPct == null ? "Off" : `${recipe.takeProfitPct}%`,
      },
      {
        label: "Stop",
      ...
      },
      {
        label: "Trailing",
        value:
          recipe.trailingPct == null ? "Off" : `${recipe.trailingPct}%`,
      },
    ];
  }
  return [
    ...
    { label: "Name", value: recipe.name },
    { label: "Contract", value: recipe.symbol },
    { label: "Action", value: recipe.formAction },
    { label: "Size", value: `${recipe.size} ${recipe.sizeUnit}` },
    {
      label: "When",
      value: `${recipe.triggerCompare} ${recipe.triggerPrice}`,
    },
    {
      label: "Take profit",
      value:
